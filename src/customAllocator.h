...
ColoringState initState(Graph<int>& g, const int v);              // Incialização do parametro state
bool greedyDSatur(Graph<int>& g, const int k, std::vector<int>& colors, std::vector<bool>& spilled);          // Abordagem hibrida greedy com decisão baseada em DSatur
bool customAllocate(Graph<int>& g, const int k, std::vector<int>& colors, std::vector<bool>& spilled);        // Algoritmo final junta todas as funções
