...
struct ColoringState {
    ...
    std::vector<std::set<bool>> neighborColors;
...
bool isColorAvailable(const ColoringState& state, int webId, int color);
void updateSaturation(Graph<int>& g, const ColoringState& state, int webId, int color);         // Update da saturação dos vizinhos do vértice
bool backtrackDSatur(Graph<int>& g, int k, ColoringState& state);           // Algoritmo DSatur + backtracking (Escolher o vértice com maior saturação, combinando um aboradegem de backtracking)
